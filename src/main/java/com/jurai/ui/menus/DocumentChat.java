        ...
        getChildren().addAll(
            new VGroup().wSpacing(8).wChildren(
                new HGroup().wAlignment(Pos.CENTER_LEFT).wSpacing(12).wChildren(
                    ...
                    new FButton("Deletar chat").wStyleClass("red-button").onAction(e -> vm.deleteChat())
                ...
                if (msg.AIMessage()) { // AI message
                    ...
                    children.add(
                        new VGroup().wChildren(
                            ...
                            new HGroup().justifyEnd().wChildren(
                                new FButton("").applyCustomFunction(btn -> {

                                    SVGPath trashIcon = new SVGPath();
                                    trashIcon.setFill(null);
                                    trashIcon.setStroke(Color.BLACK);
                                    trashIcon.setStrokeWidth(2);
                                    try {
                                        trashIcon.setContent(FileUtils.getResourceContent("/paths/trash.path"));
                                    } catch (IOException e) {
                                        throw new RuntimeException(e);
                                    }

                                    btn.setGraphic(trashIcon);
                                    btn.setStyle("-fx-background-color: transparent; -fx-padding: 4; -fx-border-color: transparent;");
                                })
                            )*/
                        )
                    );
